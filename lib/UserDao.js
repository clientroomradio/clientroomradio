...
module.exports = class UserDao extends events.EventEmitter {
    constructor(dataStore, lastfmClient, socket, config, logger) {
        ...
        dataStore.record(this, "change", "users");
    ...
    getUserConfig(user) {
        return {
            ...
            "active": user.active,
            // private, but we only send this to the user that's logged in as this
            "session": user.private.session
        };
    };
    ...
    isRadioUsername(username) {
        var user = this.users[username];
        return user.private.sk && user.active && user.allowed;
    };
    ...
    setAnonymousByUsername(username) {
        if (this.users.hasOwnProperty(username)) {
            this.users[username].private.sk = null;
            this.usersChanged();
        }
    };
    ...
    getUserBySession(session) {
        var usernames = Object.keys(this.users).filter(username => {
            return this.users[username].private.session === session;
        });
    ...
    getFilteredUsers() {
        ...
        var filteredUsers = JSON.parse(JSON.stringify(this.users, (key, value) => {
            // don't stringify conn, sk, or session
            return key === "private" ? undefined : value;
        }));

        // now remove the anonymous users
        Object.keys(filteredUsers).forEach(username => {
    ...
    broadcast(type, data) {
        Object.keys(this.users).forEach(username => {
            ...
            var userAnonymous = !user.private.sk || !user.allowed;
