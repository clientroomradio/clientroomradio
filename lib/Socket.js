    ...
    this.sockjs.on("connection", conn => {
      conn.once("data", dataAsString => {
        ...
        if (payload && payload.type === "token") {
          ...
          // This should never happen, but in theory it would be a client sending something
          // other than "login" or "token" as their first message. just send them a disconnect
  ...
  sendToConnection(conn, type, data) {
    if (conn) {
      conn.write(JSON.stringify({type: type, data: data}, (key, value) => {
        ...
        return ["sk", "conn", "interval", "callback"].indexOf(key) === -1 ? value : undefined;
      }));
    }
  }
