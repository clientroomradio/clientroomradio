module.exports = function(userDao, permissionManager, logger) {
    var that = this;
    ...
    sockjs.on("connection", function (conn) {
        ...
        conn.once("data", function (session) {
            ...
            if (session !== "") {
                try {
                    logger.winston.info("session", session);
                    var sessionJSON = JSON.parse(session);
                    logger.winston.info("session", sessionJSON);
                    var username = sessionJSON.session.name;
                    var key = sessionJSON.session.key;

                    if (permissionManager.isAllowedToJoin(username)) {
