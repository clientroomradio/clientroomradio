...
module.exports = function(config, logger, dataStore) {
    ...
    that.login = function(token, callback) {
        ...
        lastfm.session({token: token}).on("success", function (session) {
            ...
            logger.error("lastfm token error", error.message, error.stack);
            callback(error, "");
        });
    };
    ...
    function doUpdateNowPlaying(user, track) {
        lastfm.request("track.updateNowPlaying", {
            ...
            handlers: {
                success: function(/*lfm*/) {
                    logger.info("Updated now playing for", user.username);
                },
                error: function(error) {
                    logger.error("doUpdateNowPlaying", error.message);
                }
            }
        });
    }
    ...
    that.updateNowPlaying = function(track, scrobbleUsers) {
        addPlayedTrack(track);

        if (Object.keys(track).length !== 0) {
            // always scrobble to clientroom
            if (typeof config.scrobbleToHost === "undefined" || config.scrobbleToHost) {
                doUpdateNowPlaying({"username": "clientroom", "sk": config.sk}, track);
            }

            Object.keys(scrobbleUsers).forEach(function (username) {
                doUpdateNowPlaying(scrobbleUsers[username], track);
            });
        }
    };
    ...
    function doScrobble(username, sessionKey, track) {
        var options = {
            ...
            handlers: {
                success: function(lfm) {
                    logger.info("Scrobbled track for:", username, lfm);
                },
                error: function(error) {
                    logger.info("Scrobble error:" + error.message);
                }
            }
        };
    ...
    that.scrobble = function(track, scrobbleUsers, skippers) {
        if ( Object.keys(track).length !== 0 && new Date().getTime() - track.timestamp > Math.round( (track.duration * 1000) / 2 ) ) {
            ...
            Object.keys(scrobbleUsers).forEach(function (username) {
                if (skippers.indexOf(username) === -1) {
                    ...
                    doScrobble(scrobbleUsers[username], scrobbleUsers[username].sk, track);
                }
            });
        }
    };
    ...
    that.getContext = function(track, usernames, callback) {
        track.context = {};
        ...
        usernames.forEach(function (username) {
            lastfm.request("track.getInfo", {
                ...
                handlers: {
                    success: function(lfm) {
                        logger.info("getContext", username, track.name, lfm.track.userplaycount);
                        if (typeof lfm.track.album !== "undefined") {
                    ...
                    error: function(error) {
                        logger.error("getContext:track.getInfo", error.message);
                        callback(track);
                    }
                }
            });
    ...
    */
    ...
    that.getPlaylist = function(users, callback) {
        ...
        request.get(stationUrl, function (error, response, body) {
            if (error) {
                logger.error("getPlaylist error", error.message);
                logger.info("Try again in one second...");
                setTimeout(that.getPlaylist, 1000, users, callback);
            } else if (response.statusCode !== 200) {
                logger.error("getPlaylist not 200", response.statusCode);
                logger.info("Try again in one second...");
                setTimeout(that.getPlaylist, 1000, users, callback);
            } else {
