...
module.exports = class Backend {
    constructor(userDao, currentTrackManager, lastfmClient, spotify, skipManager, socket, chat, logger) {
        ...
        skipManager.on("change", (data) => {
            logger.info("skippers changed", data.skippers);

            if (userDao.getRadioUsernames().length > 0
                    && data.skippers.length > 0
                    && data.skippers.length >= data.skipLimit) {
                chat.skipSuccessful(data.skippers);
                this.onEndTrack();
            }
        });
        ...
        spotify.on("end", () => this.onEndTrack());
    }
    ...
    playTrack() {
        ...
        if (nextTrack.hasOwnProperty("request")) {
            ...
            var spotifyPlayLinks = nextTrack.playlinks.filter(playlink => {
    ...
    startRadio(radioUsernames) {
        ...
        if (this.spotify.isLoggedIn() && !this.spotify.isPlaying() && radioUsernames.length > 0) {
