...
module.exports = function(userDao, currentTrackManager, lastfm, spotify, skippers, socket, chat, logger) {
    ...
    function playTrack() {
        ...
        if (nextTrack.hasOwnProperty("request")) {
            spotify.playTrack(nextTrack.request, nextTrack.username, handlers);
        } else {
            ...
            if (spotifyPlayLinks.length > 0) {
                ...
                spotify.playTrack("spotify:track:" + spotifyPlayLinks[0].id, undefined, handlers, nextTrack);
            } else {
    ...
    function startRadio(radioUsernames) {
        if (!vlc.mediaplayer.is_playing && spotify.isLoggedIn()) {
