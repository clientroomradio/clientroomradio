...
module.exports = function(userDao, currentTrackManager, lastfm, spotify, skippers, socket, chat, logger) {
    ...
    function onGotContext(track) {
        ...
        var activeUserCount = Object.keys(active(userDao.getUsers())).length;
    ...
    function onEndTrack() {
        ...
        lastfm.scrobble(currentTrack, userDao.getUsers(), skippers.getSkippers());
    ...
    function onUsersChanged(newUsers, oldUsers) {
        ...
        if (Object.keys(active(oldUsers)).length === 0
                && !vlc.mediaplayer.is_playing ) {
            // we've gone from no users and we're not already playing so start
            startRadio(newUsers);
        }
    }
    ...
    userDao.on("change", onUsersChanged);

    startRadio(userDao.getUsers());
