...
module.exports = function(dataStore, lastfm, spotify, config, logger) {
    ...
    function onSkippersChanged(skippers) {
        logger.winston.info("onSkippersChanged:", skippers);
    ...
    function doSend(path, payload) {
        request.post("http://localhost:3001" + path, {json: payload}, function (error, response, body) {
            if (error) {
                logger.winston.error("doSend", error);
            } else if (response.statusCode !== 200) {
                logger.winston.error("doSend: STATUS CODE != 200", response.body);
            }
        });
    }
    ...
    app.post("/request", function (req, res) {
        logger.winston.info("Got a Spotify request!", req.body);
    ...
    logger.winston.info("Listening internally on port %s", config.backendPort);

    process.on("SIGINT", function () {
        logger.winston.info( "\nShutting down!" );

        dataStore.set(CURRENT_TRACK_KEY, {});
        logger.winston.info("currentTrack cleared");
        dataStore.set(SKIPPERS_KEY, []);
        logger.winston.info("skippers cleared.");
        spotify.logout();
