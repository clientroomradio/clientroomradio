module.exports = function(config, lastfmClient, userDao, chat, permissionManager, votingManager, logger) {
    var that = this;
    ...
    that.start = function() {
        ...
        app.get("/logout", function (req, res) {
            ...
            if (user) {
                userDao.removeByUsername(user.username);
            }
            res.redirect("/");
        });
        ...
        app.get("/login", function (req, res) {
            ...
            lastfmClient.login(token, function (err, session) {
                if (err) {
                    ...
                    var allowed = permissionManager.isAllowedToJoin(session.user);
                    ...
                    if (allowed) {
                        ...
                        permissionManager.requestAccess(session.user, sessionId);
                    }
