...
module.exports = class Spotify extends require("events").EventEmitter {
  constructor(userDao, skipManager, config, logger, dataStore, spotifyWeb) {
    super();
  ...
  playResolvedTrack(spotifyUri, spTrack, requester, handlers, optionalTrack) {
    if (requester === undefined && this.isPlayedTrack(spotifyUri)) {
  ...
  playTrack(spotifyUri, requester, handlers, optionalTrack) {
    if (spotifyUri.indexOf("/track/") !== -1 || spotifyUri.indexOf(":track:") !== -1) {
      ...
      handlers.error({message: `Not a Spotify URI - ${spotifyUri}`});
    }
  }
};
